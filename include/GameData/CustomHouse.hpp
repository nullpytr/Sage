...
struct GameData::CustomHouse : Tag::Structure {
	struct IsBuildMode : Tag::Member { using type = bool&; using adapter = bool&; };
	struct HouseUnitInfo;
	struct BondInfo;
	struct UsedHouseUnitNumAll : Tag::Member { using type = u32&; using adapter = u32&; };
	struct OwnHouseUnitCount : Tag::Member { using type = span<u32>; using adapter = adapter<type>*; };
...
template <> struct Data::Structure<GameData::CustomHouse> : GameData::CustomHouse {
	IsBuildMode::type IsBuildMode;
	Structure<HouseUnitInfo> HouseUnitInfo;
	Structure<BondInfo> BondInfo;
	UsedHouseUnitNumAll::type UsedHouseUnitNumAll;
	OwnHouseUnitCount::type OwnHouseUnitCount;
